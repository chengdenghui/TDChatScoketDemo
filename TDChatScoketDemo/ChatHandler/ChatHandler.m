...
- (void)socket:(GCDAsyncSocket *)sock didReadData:(NSData *)data withTag:(long)tag
{
    ...
    if ([messageModel.messageType isEqualToString:Message_Normal]) { //普通消息类型
        messageType = ChatMessageType_Normal;
    }
    else if ([messageModel.messageType isEqualToString:Message_Validate]){ //验证消息
        messageType = ChatMessageType_Validate;
    }
    else if ([messageModel.messageType isEqualToString:Message_System]){  //系统消息
        messageType = ChatMessageType_System;
    }
    else if ([messageModel.messageType isEqualToString:Message_NormalReceipt]){ //发送普通消息回执
        messageType = ChatMessageType_NormalReceipt;
        
    }
    else if ([messageModel.messageType isEqualToString:Message_LoginReceipt]){ //登录成功回执
        messageType = ChatMessageType_LoginReceipt;
    }
    else if ([messageModel.messageType  isEqualToString:Message_InvalidReceipt]){  //发送普通消息失败回执
        messageType = ChatMessageType_InvalidReceipt;
    }
    else if ([messageModel.messageType isEqualToString:Message_RepealReceipt]){  //撤销消息回执
...
-(void)socket:(GCDAsyncSocket *)sock didConnectToHost:(NSString *)host port:(uint16_t)port
{
    ...
    [settings setObject:@"此处填写服务器IP地址" forKey:GCDAsyncSocketSSLPeerName];
    [_chatScoket startTLS:settings];
}
...
-(void)socketDidSecure:(GCDAsyncSocket *)sock
{
   //登录服务器
    ...
    //此版本号需要和后台协商, 便于后台进行版本控制
    loginModel.versionCode = TCP_VersionCode;
    //当前用户ID
    loginModel.fromUserID = @"当前用户ID";
    //设备类型
...
{
    [_chatScoket readDataToData:[GCDAsyncSocket LFData] withTimeout:timeOut maxLength:0 tag:tag];
}
